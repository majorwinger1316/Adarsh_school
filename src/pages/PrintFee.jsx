...
function PrintFee() {
  ...
  const fetchDetailsByScholarNumber = async () => {
    try {
      const student = await invoke("fetch_student_by_scholar_number", {
        scholarNumber: parseInt(scholarNumber, 10),
      });
      const fees = await invoke("fetch_fee_by_scholar_number", {
        scholarNumber: parseInt(scholarNumber, 10),
      });
  ...
  const fetchDetailsByDateRange = async () => {
    try {
      const fees1 = await invoke("fetch_fee_by_date_range", {
        startDate,
        endDate,
      });
  ...
  // Add this to your PrintFee component
  const printReceipt = () => {
  ...
  return (
    <div className="print_fee">
      <div className="print_using_scholar no-print">
        <div className="title">
